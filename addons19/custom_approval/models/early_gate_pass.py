...
class CustomEarlyGatePass(models.Model):
    """Early Logout Gate Pass Request Model."""
    _name = 'custom.early.gate.pass'
    ...
    approver_ids = fields.One2many(
        'approval.approver.line', 'early_gate_pass_id',
        string='Approver(s)',
    ...
    def _compute_employee_code(self):
        """Pick up employee code from hr.employee (fallbacks for various fields)."""
        for rec in self:
            emp = rec.employee_id
            code = False
            if emp:
                # Try common fields used by HR modules to store employee code
        ...
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'custom.early.gate.pass') or _('New')
    ...
    def action_print_gate_pass(self):
        ...
        return self.env.ref(
            'custom_approval.action_report_early_gate_pass'
        ).report_action(self)
