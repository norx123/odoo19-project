...
class SignTemplate(models.Model):
    ...
    tag_ids = fields.Many2many("custom.sign.tag", string="Tags")
    ...
    sign_request_ids = fields.One2many(
        "custom.sign.request", "template_id", string="Sign Requests",
    )
    ...
    sign_item_ids = fields.One2many(
        "custom.sign.item", "template_id", string="Sign Items",
        copy=True,
    )
    ...
    def save_sign_items(self, items):
        ...
        """
        self.ensure_one()
        SignItem = self.env["custom.sign.item"]
    ...
    def action_send(self):
        self.ensure_one()
        wizard = self.env["custom.sign.send.wizard"].create({"template_id": self.id})
        return {
            "type": "ir.actions.act_window",
            "name": "New Signature Request",
            "res_model": "custom.sign.send.wizard",
            "res_id": wizard.id,
            "view_mode": "form",
            "target": "new",
    ...
    def action_sign_now(self):
        ...
        self.ensure_one()
        request = self.env["custom.sign.request"].create({
            "template_id": self.id,
            "state": "sent",
            "signer_ids": [(0, 0, {
    ...
    def action_share(self):
        self.ensure_one()
        wizard = self.env["custom.sign.share.wizard"].create({"template_id": self.id})
        return {
            "type": "ir.actions.act_window",
            "name": "Share Document",
            "res_model": "custom.sign.share.wizard",
            "res_id": wizard.id,
            "view_mode": "form",
            "target": "new",
        }
