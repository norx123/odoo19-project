...
class SignDoc(models.Model):
    ...
    def action_share(self):
        ...
        share_link = "%s/odoo/action-%d/%d" % (
            base_url,
            self.env.ref("custom_sign.action_sign_doc").id,
            self.id,
        )
        ...
        return {
            ...
            "name": "Share Document",
            "res_model": "custom.sign.doc.share.wizard",
            "view_mode": "form",
            "target": "new",
            "context": {
...
class SignDocShareWizard(models.TransientModel):
    _name = "custom.sign.doc.share.wizard"
    _description = "Share Document Wizard"

    doc_id = fields.Many2one("custom.sign.doc", string="Document", readonly=True)
