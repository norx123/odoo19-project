...
class SignRequest(models.Model):
    ...
    template_id = fields.Many2one(
        "custom.sign.template", string="Template",
    ...
    signer_ids = fields.One2many(
        "custom.sign.request.signer", "request_id", string="Signers",
    )
    ...
    sign_item_ids = fields.One2many(
        "custom.sign.item", "request_id", string="Sign Items",
    )
    ...
    def create(self, vals_list):
        ...
        for rec in records:
            ...
            if not rec.signer_ids:
                self.env["custom.sign.request.signer"].create({
                    "request_id": rec.id,
            ...
            if rec.template_id and not rec.sign_item_ids:
                for tpl_item in rec.template_id.sign_item_ids:
                    self.env["custom.sign.item"].create({
                        "request_id": rec.id,
...
class SignRequestSigner(models.Model):
    _name = "custom.sign.request.signer"
    _description = "Sign Request Signer"
    _order = "id asc"

    request_id = fields.Many2one(
        "custom.sign.request", string="Request",
        required=True, ondelete="cascade",
    )
