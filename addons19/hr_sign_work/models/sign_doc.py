...
class SignDoc(models.Model):
    ...
    def action_share(self):
        ...
        share_link = '%s/web#model=sign.doc&id=%s' % (base_url, self.id)
...
class SignDocShareWizard(models.TransientModel):
    ...
    partner_ids = fields.Many2many(
        'res.partner',
        'sign_doc_share_wizard_partner_rel',
        'wizard_id',
        'partner_id',
        string='Share With',
    )
    ...
    def action_done(self):
        if self.doc_id and self.partner_ids:
            self.doc_id.shared_with_ids = [(4, p.id) for p in self.partner_ids]
