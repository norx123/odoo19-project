...
(function () {
    ...
    let currentTemplateId = null;     // Active sign.template record
    let fieldTypes = [];              // sign.item.type list
    ...
    async function fetchFieldTypes() {
        try {
            return await rpc("sign.item.type", "search_read", [[]], {
    ...
    function tryRenderSavedPdf() {
        ...
        rpc("sign.template", "read",
            ...
            const url = "/web/content?model=sign.template&id=" + rec.id +
    ...
    async function saveFieldsToBackend() {
        ...
        try {
            await rpc("sign.template", "save_sign_items",
